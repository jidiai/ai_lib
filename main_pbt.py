...
def main():
    ...
    assert cfg.training_manager.batch_size<=cfg.data_server.episode_capacity,"batch_size({}) should be <= episode_capacity({})".format(cfg.training_manager.batch_size,cfg.data_server.episode_capacity)

    
    timestamp=time.strftime("%Y-%m-%d-%H-%M-%S",time.localtime())
    cfg.expr_log_dir=os.path.join(cfg.log_dir,cfg.expr_group,cfg.expr_name,timestamp)
    log_path = os.path.join(BASE_DIR,cfg.expr_log_dir)
    print('log path=', log_path)
    print('Base=', BASE_DIR)
    print('exp log dir = ', cfg.expr_log_dir)
