...
class Refrigerator(Game):
    ...
    def step(self, joint_action):
        ...
        if self.step_cnt % self.open_interval == 0:
            ...
            self.door_open_after_step = random.randint(
                0, self.open_interval - self.door_open_agent.door_open_time
            )
        action = joint_action[0][0]
        if (
            self.open_door
            and self.step_cnt % self.open_interval >= self.door_open_after_step
        ):
            door_open = self.door_open_agent.act()
            self.cool_simulator.update(
                power=action, dt=self.sampling_time, door_open=door_open
            )
        else:
            door_open = False
            self.cool_simulator.update(
    ...
    def get_all_observes(self):
        return [
            {"obs": copy.deepcopy(self.current_state), "controlled_player_index": 0}
        ]
    ...
    def get_info_after(self, door_open):
        return {
            "temperature": copy.deepcopy(self.current_state),
            "controlled_player_index": 0,
            "door_open": door_open,
        }

    def set_n_return(self):
        self.n_return[0] = -np.mean(
            np.abs(np.array(self.traj) - self.target_temperature)
        )
