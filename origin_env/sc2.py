...
class SC2(Game):
    def __init__(self, conf):
        ...
        self.players = [
            sc2_env.Agent(sc2_env.Race[agent_type]) for agent_type in conf["agent_type"]
        ]

        self.env_core = sc2_env.SC2Env(
    ...
    def is_valid_action(self, joint_action):
        ...
        for i in range(self.n_player):
            if (
                joint_action[i][0].function
                not in self.joint_action_space[i][0].available_actions
            ):
                raise Exception(
                    "The input action dimension for player {} should be {}, does not have {}".format(
                        i,
                        self.joint_action_space[i][0].available_actions,
                        joint_action[i][0].function,
                    )
                )
