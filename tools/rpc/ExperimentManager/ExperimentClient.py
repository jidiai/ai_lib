...
from tools.rpc.chunk import serialize, binary_chunks
from tools.rpc.proto import exprmanager_pb2, exprmanager_pb2_grpc
from tools.rpc.chunk import serialize_pyplot
    ...
    def _send_scalar(server_addr, key, tag, nid, content, step, time):
        with grpc.insecure_channel(server_addr) as channel:
            ...
            if hasattr(scalar, op_name):
                ...
                print(
                    f"Unknown type: {content_type_name} can not be converted to scalar, tag = {tag}"
                )
