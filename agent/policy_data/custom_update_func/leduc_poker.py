...
def update_func(policy_data_manager, eval_results, **kwargs):
    ...
    policy_ids_0 = [
        policy_id.split("_")[-1] for policy_id in policy_ids_dict["agent_0"]
    ]
    policy_ids_1 = [
        policy_id.split("_")[-1] for policy_id in policy_ids_dict["agent_1"]
    ]
    ...
    table = pformat_table(
        payoff_matrix, headers=policy_ids_1, row_indices=policy_ids_0, floatfmt="+4.2f"
    )
    ...
    training_agent_id = policy_data_manager.agents.training_agent_ids[0]
    exploitability_array = policy_data_manager.get_array_data("exploitability")[
        training_agent_id
    ]
    missing_indices = np.nonzero(
        exploitability_array
        == policy_data_manager.cfg.fields.exploitability.missing_value
    )[0]
    missing_policy_ids = [
        policy_data_manager.agents[training_agent_id].policy_ids[missing_index]
        for missing_index in missing_indices
    ]
    missing_policy_descs = ray.get(
        [
            policy_data_manager.policy_server.pull.remote(
                policy_data_manager.id, training_agent_id, policy_id
            )
    ...
    for policy_id, policy in zip(missing_policy_ids, missing_policies):
        ...
        policy_data_manager.data["exploitability"][training_agent_id][
            idx
        ] = exploitability
    ...
    Logger.info(
        "last {} policies' exploitabilities are:\n{}".format(
            last_k,
            pformat_table(
                exploitability_array,
                headers=["policy_id", "exploitability"],
                row_indices=policy_data_manager.agents[training_agent_id].policy_ids[
                    -last_k:
                ],
                floatfmt="+6.2f",
            ),
    ...
    for agent_id in policy_data_manager.agents.keys():
        ...
        agent_id2policy_indices[agent_id] = np.array(
            [
                policy_data_manager.agents[agent_id].policy_id2idx[policy_id]
                for policy_id in population.policy_ids
            ]
        )

    # get payoff matrix
    payoff_matrix = policy_data_manager.get_matrix_data(
        "payoff", agent_id2policy_indices
    )

    # compute nash
    equlibrium_distributions = NashSolver().compute(payoff_matrix)
    # print('equilibrium distribution = ', equlibrium_distributions)
    ...
    # print('policy distribution = ', policy_distributions)
    ...
    monitor = get_actor(policy_data_manager.id, "Monitor")
    ray.get(
        monitor.add_scalar.remote(
            "PSRO/Nash_Equilibrium/Exploitability",
            nash_eq_exploitability,
            len(policy_data_manager.nash_eq_exploitabilities) - 1,
        )
    )
    ray.get(
        monitor.add_scalar.remote(
            "PSRO/Nash_Equilibrium/Exploitability_per_training_step",
            nash_eq_exploitability,
            kwargs["extra_results"]["total_training_steps"],
        )
    )

    try:
        Logger.info(
            "last {} nash eq' exploitabilities are:\n{}".format(
                last_k,
                pformat_table(
                    np.array(
                        policy_data_manager.nash_eq_exploitabilities[-last_k:]
                    ).reshape(-1, 1),
                    headers=["policy_id (nash eq)", "exploitability"],
                    row_indices=policy_data_manager.agents[
                        training_agent_id
                    ].policy_ids[-last_k:],
                    floatfmt="+6.2f",
                ),
...
def print_policy(policy):
    ...
    for state, probs in zip(
        it.chain(*policy.states_per_player), policy.action_probability_array
    ):
...
def get_nash_equilibrium_policy(policies, probs):
    ...
    policy = aggregate_policies(
        game,
        total_policies=[policies, policies],
