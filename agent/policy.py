...
from tools.utils.typing import DataTransferType, Tuple, Any, Dict, EpisodeID, List
...
class MAPPO(Policy):
    def __init__(
        ...
        model_type = model_config.get("model", "basic")
        if model_type == 'sdsgscl':
            self.random_exploration = 0.2

        # Logger.warning("use model type: {}".format(model_type))
        model=importlib.import_module("malib.algorithm.mappo.models.{}".format(model_type))
    ...
    def compute_action(self, **kwargs):
        observation=kwargs[EpisodeKey.CUR_OBS]
        ...
        raise NotImplementedError("about cur state")
        if EpisodeKey.CUR_STATE in kwargs and kwargs[EpisodeKey.CUR_STATE] is not None:
            # print('state shape = ', kwargs[EpisodeKey.CUR_STATE].shape)
            # raise NotImplementedError
