...
parser.add_argument("--env_name", default="classic_Pendulum-v0")  # OpenAI gym environment name
...
class SAC():
    ...
    def update(self):
        ...
        for _ in range(args.gradient_steps):
            ...
            # # print('2', type(bn_s), type(bn_a))
            # bn_s = bn_s.to(torch.float32)
            # bn_a = bn_a.to(torch.float32)
...
def main():
    ...
    for i in range(args.iteration):
        ...
        for t in range(200):
            ...
            action = action + np.random.normal(0, 0.1, size=env.action_dim.shape[0])
