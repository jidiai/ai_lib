...
class TD3:
    ...
    def learn(self):
        ...
        for i in range(self.update_freq):
            ...
            done = torch.tensor(transitions["d_0"], dtype=torch.float).view(
                self.batch_size, -1
            )
    ...
    def save(self, save_path, episode):
        ...
        model_critic_1_path = os.path.join(
            base_path, "critic_1" + str(episode) + ".pth"
        )
        torch.save(self.critic_1.state_dict(), model_critic_1_path)
        model_critic_1_target_path = os.path.join(
            base_path, "critic_1_target" + str(episode) + ".pth"
        )
        torch.save(self.critic_1_target.state_dict(), model_critic_1_target_path)

        model_critic_2_path = os.path.join(
            base_path, "critic_2" + str(episode) + ".pth"
        )
        torch.save(self.critic_2.state_dict(), model_critic_2_path)
        model_critic_2_target_path = os.path.join(
            base_path, "critic_2_target" + str(episode) + ".pth"
        )
        ...
        model_actor_target_path = os.path.join(
            base_path, "actor_target" + str(episode) + ".pth"
        )
