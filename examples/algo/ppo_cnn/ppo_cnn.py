...
class PPO_CNN(object):
    def __init__(self, args):
        ...
        if self.is_matrix:
            ...
            self.encoder_actor_optimizer = optim.Adam(
                self.encoder_a.parameters(), lr=self.a_lr
            )
            self.encoder_critic_optimizer = optim.Adam(
                self.encoder_c.parameters(), lr=self.c_lr
            )
        else:
    ...
    def learn(self):
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(
                ...
                if self.is_matrix:
                    action_prob = self.actor(self.encoder_a(obs[index])).gather(
                        1, action[index]
                    )
                else:
                ...
                nn.utils.clip_grad_norm_(
                    self.encoder_a.parameters(), self.max_grad_norm
                )
                ...
                nn.utils.clip_grad_norm_(
                    self.encoder_c.parameters(), self.max_grad_norm
                )
