...
class ParticleEnv_simple_spread(BaseWrapper):
    ...
    def get_actionspace(self):
        action_dim = [self.env.get_single_action_space(id)[0].n for id in range(self.env.n_player)]
        print('##', action_dim)
        return action_dim

    def get_observationspace(self):
        obs_dim = [self.env.current_state[id].shape[0] for id in range(self.env.n_player)]
        print('##', obs_dim)
        return obs_dim
