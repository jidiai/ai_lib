...
class Runner:
    ...
    def get_joint_action_eval(
        self, game, multi_part_agent_ids, policy_list, actions_spaces, all_observes
    ):
        joint_action = []
        ...
        for i_epoch in range(1, self.paras.max_episodes + 1):
            ...
            while not self.g_core.is_terminal():
                ...
                joint_act = self.get_joint_action_eval(
                    self.env, multi_part_agent_ids, self.policy, actions_space, state
                )
            ...
            self.writer.add_scalars(
                reward_tag, global_step=i_epoch, tag_scalar_dict={"return": Gt}
            )
