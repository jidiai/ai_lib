...
def get_players_and_action_space_list(g):
    if sum(g.agent_nums) != g.n_player:
        raise Exception(
            "agent number = %d 不正确，与n_player = %d 不匹配" % (sum(g.agent_nums), g.n_player)
        )
...
def render_game(g, fps=1):
    ...
    game_info = dict(
        ...
        init_info=str(g.init_info),
        start_time=st,
        mode="window",
        render_info={
            "color": g.colors,
            "grid_unit": g.grid_unit,
            "fix": g.grid_unit_fix,
        },
    )
    ...
    while not g.is_terminal():
        ...
        print(step)
        game_info[step] = {}
        game_info[step]["time"] = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(time.time())
        )
        joint_act = get_joint_action_eval(
            g, multi_part_agent_ids, policy_list, actions_space, all_observes
        )
