...
from utils.naming import default_table_name, EXPERT_DATA_TABLE_NAME
...
from training import distributed_trainer, data_prefetcher
import ray
...
class BCTrainingManager:
    def __init__(self, id, cfg):
        ...
        self.prefetchers = [
            DataPrefetcher.options(max_concurrency=10).remote(
                self.cfg.data_prefetcher, self.trainers, [self.shared_data_server]
            )
    ...
    def train(self, training_desc: TrainingDesc):
        ...
        expert_data_table_name = EXPERT_DATA_TABLE_NAME

        # TODO(jh): we don't need simulation data in BC
        # create table
        ...
        prefetching_desc = PrefetchingDesc(
            expert_data_table_name, self.cfg.batch_size.expert_data
        )
        ...
        while True:
            ...
            for key, value in training_statistics.items():
                tag = "Training/{}/{}/{}".format(
                    training_desc.agent_id, training_desc.policy_id, key
                )
                log_task = self.monitor.add_scalar.remote(
                    tag, value, global_step=training_steps
                )
            ...
            data_server_statistics = ray.get(
                self.shared_data_server.get_statistics.remote(expert_data_table_name)
            )
