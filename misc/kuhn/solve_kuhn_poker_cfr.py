...
class CFRSolver:
    ...
    def index_state(self,player,state):
        return "".join([str(int(num)) for num in state.information_state_tensor(player)])
    ...
    def traverse(self,state,reaching_probs):
        ...
        if state.is_terminal():
            ...
            state_action_values=np.zeros((self.num_actions,self.num_players),dtype=np.float32)
            ...
            cfr_prob = np.prod(reaching_probs[:player])*np.prod(reaching_probs[player+1:])
            ...
            self.regrets[index]+=cfr_prob*legal_action_mask*(state_action_values[:,player]-state_values[player])
