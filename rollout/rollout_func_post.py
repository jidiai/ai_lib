...
def rollout_func(
    ...
    **kwargs
    ...
    if not hasattr(env, "post_rewarder"):
        ...
        env.post_rewarder = PostRewarder(
            path="expr_log/log/gr_football_rewarder/just_test/2022-09-25-23-36-17/rewarder_10.pt"
        )
    ...
    if not eval:
        ...
        # assign post rewards
        post_rewards, post_dones = env.post_rewarder.cal_rewards(env.tracer)
        episode[EpisodeKey.REWARD] += post_rewards
        episode[EpisodeKey.DONE] |= post_dones
        # Logger.error("{}\n{}".format(episode[EpisodeKey.REWARD][:15,8],episode[EpisodeKey.DONE][:15,8]))
        env.stats_calculators[rollout_desc.agent_id].stats["reward"] += np.sum(
            post_rewards
        )
