    ...
    def _rollout_loop(self, rollout_desc: RolloutDesc):
        ...
        while True:
            ...
            with self.rollout_epoch_lock:
                rollout_epoch = self.rollout_epoch
            self.worker_pool.submit(
                lambda worker, v: worker.rollout.remote(rollout_desc, eval=False, rollout_epoch=rollout_epoch),
                value=None
            )
            submit_ctr += 1
    ...
    def rollout(self, rollout_desc: RolloutDesc):
        ...
        try:
            ...
            while True:
                ...
                results, timer_results = self.get_batch(self.data_buffer, self.data_buffer_lock, self.data_buffer_ready,
                                                        self.batch_size)
                ...
                if rollout_desc.sync and rollout_epoch % self.eval_freq == 0:
                    ...
                    ray.get(
                        self.monitor.add_multiple_scalars.remote(main_tag, timer_results, global_step=rollout_epoch))
