...
class RolloutManager:
    ...
    def rollout(self,rollout_desc:RolloutDesc):
        ...
        try:
            best_reward=-np.inf
            wins=deque([0]*20,maxlen=20)
            rewards=deque([-np.inf]*20,maxlen=20)
            while True:
                with self.rollout_epoch_lock:
                    if self.rollout_epoch>=max_rollout_epoch or np.mean(wins)>0.9:
                        break
                    self.rollout_epoch+=1
                    rollout_epoch=self.rollout_epoch
                global_timer.record("batch_start")
                results,timer_results=self.get_batched_results()
                ...
                if reward>=best_reward:
                    Logger.warning(f"save the best model(average reward:{reward})")
                    best_reward=reward
