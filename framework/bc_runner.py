...
class BCRunner:
    def __init__(self, cfg):
        ...
        )(training.BCTrainingManager)
        DataServer = ray.remote(**get_resources(cfg.data_server.distributed.resources))(
            buffer.DataServer
        )
        SharedDataServer = ray.remote(
            **get_resources(cfg.shared_data_server.distributed.resources)
        )(buffer.DataServer)
        ...
        self.shared_data_server = SharedDataServer.options(
            ...
            max_concurrency=500,
        ).remote("SharedDataServer", self.cfg.shared_data_server)
        ...
        if self.id == "psro":
            ...
            assert (
                len(population_cfgs) == 1
                and population_cfgs[0]["population_id"] == "default"
            )
    ...
    def load_expert_data(self):
        ...
        ray.get(
            self.shared_data_server.load_data.remote(
                EXPERT_DATA_TABLE_NAME, self.cfg.shared_data_server.preload_data_path
            )
        )
